...
- Go 1.22+ ServeMux uses {name} for path parameters, not :name. Example: mux.HandleFunc("GET /projects/{id}", handler). Access with r.PathValue("id").
- httptest.ResponseRecorder does NOT implement http.Flusher. For SSE/streaming tests, use httptest.NewServer with a real HTTP client, or use httptest.NewRecorder only for non-streaming endpoints.
- For SSE test cleanup, use context.WithCancel and cancel after reading expected events. Do not rely on timeouts or sleep for test termination.`
